...
(defun body () (list 
		...
		(make-p body '"Abstract: Lfeweb is using Lfe (Lisp Flavoured Erlang) to add lisp syntax to Nitrogen. Lfe is created by Roberg Virding and described as a lisp syntax front-end to the Erlang compiler. Nitrogen is a web framework created by Rusty Klophaus that nicely put javascript, comet and jquery functionality in the background, making it easier to work on an even higher abstraction level than before. This page demonstrate how much of Nitrogen is working using Lfe and hopefully how addicting the lisp syntax is.")
                ...
                (make-p body '"A small warning to begin with. This page is not done. If this could be a issue we recommend that you visit us in a month or two. If new things excites you then this is probably just exiting. Then welcome. Ok, Lisp is an old language with extremely simple consepts, that we can build upon. If someone say Lisp is simple. That is true. But that doesn't mean that all you build with lisp will be simple. It requires courage to build simple things with simple parts. That is the most respectable thing you can do in software. Let's go on and install Lfeweb, to see how simple things look like.")
