...
(defun body () (list 
                ...
                (make-p body '"Lfeweb depends on Lfe and Nitrogen. Lfe and Nitrogen depends on Erlang. Therefor we start with the installation of Erlang. Erlang works on every operation system out there. We recommend testing it out on the latest Ubuntu.")

                (make-p body '"<pre>apt-get intall erlang</pre>")
		(make-p body '"Then we need Lfe and Nitrogen.")
                (make-p body '"<pre>
mkdir ~/elib && cd ~/elib
...
source ~/.bashrc
                ...
                (make-p body '"Before we install lfeweb we need to create a nitrogen project. Then add lfeweb inside the project folder.")
...
curl localhost:8000/web/blog
                ...
                (make-p body '"My dev setup is a small experiment. At the moment I use flymake to trigger the refresh _and_ the auto reload of beam-files. It's not 100% reliable, but it's ok. I don't have to hit the browser refresh button all the time (Instead I must restart flymake-mode when it flips out). To make use of auto-refresh we must install mozrepl for firefox and then screen.")
                (make-link url '"http://wiki.github.com/bard/mozrepl" text '"Download page for mozrepl")
                (make-p body '"After installation of mozrepl, make sure you enable it in the menu. Then enter moz-minor-mode in emacs and hit C-c C-s to get a moz-prompt. Then you know you got contact with your browser. To refresh the browser you can hit C-x p or make sure your makefile send refresh to your repl.")
                ...
                (make-p body '"On the server I use Nginx, a rock solid reverse proxy. Perfect for mapping the url to the folders on your disk. For static files, it is extremely fast. Then after the Nginx setup a usually install mochiweb and enables it in the ./ebin/myproject.app file. ")
